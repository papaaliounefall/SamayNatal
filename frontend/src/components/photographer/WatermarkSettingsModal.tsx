...
const CROPPABLE_POSITIONS: WatermarkPosition[] = ['bottom-right', 'bottom-left', 'top-right'];
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Filigrane & Protection d'images" subtitle={`Configuration pour l'événement : ${event.title}`} maxWidth="lg">
      <div className="space-y-5">
        <div className="flex items-center justify-between p-3 bg-[#F8F9FA] rounded-lg border border-[#E5E7EB]">
          ...
          <input
            type="checkbox"
            aria-label="Activer le filigrane automatique"
            checked={enabled}
        ...
        <div>
          <label htmlFor="watermark-text" className="block text-xs font-semibold text-[#111827] mb-1">Texte du filigrane</label>
          <input
            id="watermark-text"
            type="text"
            disabled={!enabled}
        ...
        <div>
          ...
          {CROPPABLE_POSITIONS.includes(position) && (
            <p className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-md px-2.5 py-1.5 mt-2 flex items-start gap-1.5">
              <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5" />
              Un filigrane en coin se recadre facilement — "Centre" ou "Mosaïque" protègent mieux contre le vol par capture d'écran.
            </p>
          )}
        </div>
